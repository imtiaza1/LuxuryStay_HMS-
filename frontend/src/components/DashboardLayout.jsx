    ...
    switch (user.role) {
      case ROLES.ADMIN:
        return [
          ...
          { name: 'Room Management', icon: Hotel, path: '/dashboard/admin/rooms' },
          { name: 'Bookings', icon: Calendar, path: '/dashboard/admin/bookings' },
          { name: 'Reviews', icon: Star, path: '/dashboard/admin/reviews' },
          { name: 'Billing', icon: DollarSign, path: '/dashboard/admin/billing' },
          { name: 'Housekeeping', icon: ClipboardList, path: '/dashboard/admin/housekeeping' },
          { name: 'Settings', icon: Settings, path: '/dashboard/admin/settings' }
        ];
      case ROLES.MANAGER:
        return [
          ...baseItems,
          { name: 'Bookings', icon: Calendar, path: '/dashboard/manager/bookings' },
          { name: 'Housekeeping', icon: ClipboardList, path: '/dashboard/manager/housekeeping' },
          { name: 'Revenue', icon: DollarSign, path: '/dashboard/manager/revenue' },
          { name: 'Staff', icon: Users, path: '/dashboard/manager/staff' }
        ];
      case ROLES.RECEPTIONIST:
        return [
          ...baseItems,
          { name: 'Check-in/out', icon: UserCheck, path: '/dashboard/receptionist/checkin' },
          { name: 'Bookings', icon: Calendar, path: '/dashboard/receptionist/bookings' },
          { name: 'Guests', icon: Users, path: '/dashboard/receptionist/guests' },
          { name: 'Rooms', icon: Key, path: '/dashboard/receptionist/rooms' }
        ];
      case ROLES.HOUSEKEEPING:
        return [
          ...baseItems,
          { name: 'My Tasks', icon: CheckSquare, path: '/dashboard/housekeeping/tasks' },
          { name: 'Room Status', icon: Hotel, path: '/dashboard/housekeeping/rooms' }
        ];
      case ROLES.GUEST:
        return [
          ...baseItems,
          { name: 'My Bookings', icon: Calendar, path: '/dashboard/guest/bookings' },
